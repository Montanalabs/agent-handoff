...
Untrusted input is an agent message; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — an agent message
- **Closed type:** `type Decision = Send(AgentMsg) | Stop` where `AgentMsg` is closed
- **Sink / capability:** `grant handoff` → `privileged { handoff(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/agent-handoff/demo.sh
```
...
- `agent-handoff_safe.wave` — the correct design.
- `agent-handoff_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
